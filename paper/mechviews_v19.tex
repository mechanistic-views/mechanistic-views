...
Table~\ref{tab:violations} lists six phrasings in common use whose
...
``\emph{The} circuit for task~$T$''
  & one circuit under one criterion \emph{(object)}
  & the unique mechanism
  & which identity criterion is in use, and what other criteria return
    \citep{meloux2025} \\[4pt]
...
``We localized it to head~9.9'' (offered as the explanation)
  ...
  & what the component does; the object view supplies which components,
    and a role supplies what they do \\[4pt]
``Ablating it changed nothing, so it is not involved''
  & absence under one view
  & absence in general
  & whether other views support it: a view failure leaves the claim
    standing \\
\bottomrule
\end{tabular}
\caption{Six phrasings whose evidence and claim sit under different views.
