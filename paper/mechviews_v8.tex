...
\subsection{Cross-Model Identity}\label{app:cs_cross}

``The same feature in different models'' requires specifying what
``same'' means. Most universality claims use instrumental evidence (same
top-activating examples) for structural conclusions (same mechanism).

\paragraph{Instrumental view.}
``Same'' means same predictive model---the feature activates on the same
inputs across models. Weakest notion: behavioral similarity, not
mechanistic identity.

\paragraph{Object view.}
``Same'' means same component---same head index, same layer position.
Logically impossible across architectures with different sizes. This is a
hard ceiling on the Object view, not a solvable problem.

\paragraph{Subspace view.}
``Same'' means same causal subspace---measurable via Grassmannian
distance after alignment. Well-defined across architectures of different
sizes.

\paragraph{Structural view.}
``Same'' means same gauge orbit---the computations are identical up to
reparameterization. Requires parallel transport across model manifolds.
The strongest possible notion; rarely attempted.

\paragraph{Verdicts.}
\emph{Instrumental}: common but weak---same examples does not imply same
mechanism.
\emph{Object}: impossible across architectures---a hard ceiling.
\emph{Subspace}: testable---Grassmannian distance is well-defined and
architecture-independent.
\emph{Structural}: strongest but rarely attempted.
