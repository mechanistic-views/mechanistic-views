...
This paper introduces Mechanistic Views---the explicit background
assumptions behind any mechanistic claim---and uses them to answer a
question the field currently cannot: when is a mechanism claim
\emph{real}, rather than an artifact of the method that found it?
Views differ in their failure modes, and those failure modes are
largely independent: the confound that manufactures an artifact under
one view does not manufacture one under another.  A claim that survives
across views with independent failure modes is therefore a claim that
no single method's artifacts can explain.  We define its \emph{invariance
depth} as the weighted count of independent views supporting it, and
argue that high invariance depth is the operational criterion for
treating a mechanism as perspective-independent.

Each view answers five questions: what kind of entity counts as a
mechanism (ontology), when two descriptions refer to the same one
(identity), what measurements can warrant the claim (evidence), what
mathematical language expresses it (formalism), and what phenomenon is
being explained (target).  Ontology constrains identity, which
constrains formalism; evidence and target are constrained by the
ontology--identity pair but not fully determined by it.

A motivating walkthrough shows how a single claim---``head~9.9 is an
S-inhibition head''---splits into seven distinct claims under seven of
the nine views, grounded in recent empirical data on circuit overlap,
head stability, and prompt specificity.  The criterion discriminates:
the induction head mechanism is supported across four views spanning
three independent families, while a single sparse-autoencoder feature
is supported under one view and its identity is decomposition-dependent.
Surveying 17 open problems from seven sources, we find that 8 are
clarified once the view is specified, 2 are reframed once the required
evidence type is identified, and 7 are scoped as structural limits of
the view currently used to study them.  The last category matters most:
these are ceilings that cannot be transcended by better methods within
the same view.
\end{abstract}
...
circuit structures.  The signals flowing through head~9.9 show negative cosine similarity
between the two template types.
...
\item \textbf{Formalism}: what mathematical language expresses the
      claim?  (Directed graphs, Grassmannian geometry, composition
      scores, dynamical systems, etc.)
...
(subspaces); gauge orbits and weight-space composition scores (structural view); dynamical
...
Structural
  ...
  & Composition scores, reparameterization tests
  & Gauge orbit
...
Each view is developed in full in Appendix~\ref{app:viewdetail}, which states its
ontology, identity criterion, characteristic evidence, formalism, failure modes,
and the experiment that would discriminate it from its neighbours.  What matters
for the argument of this paper is not the individual entries but their grouping:
views fall into families whose failure modes differ, and that difference is what
makes agreement across views informative rather than merely repeated.
...
\item \textbf{Methodological singleton} (Perspectival): no single
      ...
      coherence.  This is not a meta-level commentary on views but a distinct ontological commitment: the mechanism \emph{is} the convergent structure across perspectives, not the object any single perspective reveals.
...
\item \textbf{Contrastive singleton} (Contrastive): mechanisms are
      ...
      patterns.  This differs from a foil-parameterized Object view because the foil is constitutive of the mechanism, not a parameter of the experiment: changing the foil changes the mechanism itself, not just the evidence about it.
\end{itemize}
...
Every commonly used interpretability method carries commitments across all five
axes, and most carry them silently.  Activation patching and ablation operate in
the object view.  Distributed alignment search and its relatives operate in the
subspace view, while being reported as though they had established a role.
Weight-based composition analysis operates in the structural view.  Three
observations follow.  First, the concentration is severe: most published work
sits in the object or role views, so the field's evidence base is narrower than
its method count suggests.  Second, a method's search space is not its ontology:
DAS searches over subspaces but its identity criterion is a role-level one,
whether two representations can be exchanged without loss of function.  Third,
no widely used method operates in the stratified or perspectival views, which at
present generate research programs rather than describing practice.  The full
method-by-axis table is in Appendix~\ref{app:viewdetail}.
...
observations.  Weight-space composition scores supply exactly such a
fingerprint, and for this circuit they have been computed: the
communication between S-inhibition and name-mover heads is recoverable
from the weights alone, without reference to activations
\citep{merullo2024}.
...
\emph{Subspace view}: attempted; alignment search identifies a
low-dimensional subspace mediating the indirect-object variable
\citep{geiger2024}.
\emph{Structural view}: attempted; weight-based composition scores have
been computed for the name-mover head~9.9 without recourse to activation
data \citep{merullo2024}.
\emph{Process view}: partially attempted; circuit-level roles persist
across training checkpoints \citep{tigges2024}, though the formation of
this circuit in particular has not been traced.
\emph{Contrastive view}: the IOI circuit as stated is foil-relative;
varying the foil changes the circuit.

\paragraph{Depth.}
IOI therefore draws support from four families, placing its raw invariance
depth above the induction head's.  What survives of the familiar criticism
is not shallow support but inconsistency: the object-view claim that the
circuit is fixed and the contrastive-view finding that it is foil- and
prompt-relative cannot both be held, so the maximal consistent subset is
smaller than the raw support set (\S\ref{sec:realism}).
...
subspace is not established.  Under the structural view: the composition
structure has been characterized in weight space \citep{elhage2021}, but
has not been tested for invariance under computation-preserving
reparameterization.  Under
...
the structural view: gauge-invariance checks on weight-space composition
structure, computed before and after reparameterization.  For the
stratified view:
...
promoted to a higher-commitment view?  The conditions below are proposed
rather than established.  No promotion has yet been carried out end to
end, so this section states a research programme, not a validated method.
\S\ref{sec:experiment} describes the experiment that would test it: an
attempt to promote sparse-autoencoder feature claims from the object view
to the subspace view, where recent evidence that individually unstable
features cluster into reproducible subspaces supplies a concrete rescue
hypothesis.  Until that experiment is run, the conditions should be read
as predictions about what promotion requires.
\begin{itemize}[leftmargin=*]
...
\item \textbf{Subspace $\to$ Structural.}  The subspace must survive
      ...
      Weight-space composition scores computed before and after
      reparameterization provide the check \citep{elhage2021,merullo2024}.
...
makes subspace proximity a metric statement; composition scores make
gauge invariance a computable check rather than an appeal; Whitney
...
%% ---------------------------------------------------------------
\section{When Is a Mechanism Real?}\label{sec:realism}
%% ---------------------------------------------------------------

Consider two mechanism claims.  The first: induction heads in GPT-2 implement
in-context copying \citep{olsson2022}.  This claim is supported by activation
patching, by an independently operationalized functional role (copying the token
after the previous occurrence), by weight-space composition scores showing QK
composition between previous-token heads and induction heads \citep{elhage2021},
and by a training-time phase transition where the mechanism forms abruptly at a
specific step.  Four lines of evidence across three independent families of
methods.

The second: a specific SAE dictionary element activates on the Golden Gate
Bridge \citep{bricken2023}.  This claim is supported by one method---sparse
autoencoder training under the object view.  Different random seeds produce
different features \citep{leask2025}.  The feature's identity is
decomposition-dependent.

The first claim feels more real than the second.  This section asks why, and
whether that feeling can be made precise.  The answer we propose is
\emph{cross-view invariance}.  The views catalogued above differ in their
failure modes, and those failure modes are independent: a confound that
generates an artifact under one view does not typically generate one under
another.  When a mechanism claim survives across views with independent failure
modes, something is being detected that no single method's artifacts can
explain.

\subsection{Validity is View-Relative}\label{sec:view-relative}

A natural reading of evidence quality treats it as a single score for a claim,
independent of which view generated the evidence.  This reading is wrong.

Consider a claim about head~9.1 in GPT-2: ``head~9.1 is a name mover (IOI
context).''  Under the object view, strong evidence exists: ablation changes
output, and the role is operationalized across inputs.  Under the subspace view,
no evidence exists---no one has shown that the relevant information is encoded
in a specific low-dimensional subspace.  Under the process view, no evidence
exists: the training dynamics of name mover heads have not been studied.

Evidence quality is not a property of the claim in isolation.  It is a property
of the claim \emph{under a specific view}.  A strong ablation result says: this
evidence is strong, under the object view's standards and subject to the object
view's failure modes.  It says nothing about the subspace view's question and
nothing about the process view's question.

The atomic unit of mechanistic knowledge is therefore the triple
$(C,\, V_i,\, \mathrm{tier}_i)$, where $C$ is a mechanism claim with specified
carrier set and role, $V_i$ is the view under which the claim is stated, and
$\mathrm{tier}_i \in \{0,1,2,3,4\}$ is the evidence quality for $C$ as assessed
under $V_i$.  A single mechanism claim as typically reported in the literature
is implicitly a bundle of such triples: one per view under which evidence has
been gathered.  Reporting a claim without specifying the view--tier pairs is
like reporting an effect size without a confidence interval: not wrong, but
incomplete.

\subsection{Invariance Depth}\label{sec:depth}

Let $\mathcal{V}$ be the nine views.  A claim $C$ has \emph{view-support set}
\[
\mathrm{supp}(C) = \{V_i \in \mathcal{V} : \mathrm{tier}(C, V_i) \geq 1
  \text{ and the } V_i\text{-description is mutually consistent with all others
  in } \mathrm{supp}(C)\}.
\]
Two descriptions are \emph{mutually consistent} when the same discriminating
experiment would confirm or disconfirm both.

If any pair of views in $\mathrm{supp}(C)$ produce mutually inconsistent
descriptions, the inconsistent views are removed and the view-support set is
recomputed on the maximal consistent subset.  A large but inconsistent support
set scores lower than a small consistent one.

Invariance depth weights each supporting view by both its family independence
and its within-view evidence quality:
\[
\delta^{*}(C) = \sum_{V_i \in \mathrm{supp}(C)} w_i \cdot
  \frac{\mathrm{tier}(C, V_i)}{4},
\]
where $w_i$ is the family-independence weight---the first view from each family
receives weight~$1$, and the $k$-th view from the same family receives weight
$1/k$---and the tier is normalized to $[0,1]$.  The maximum achievable value is
$5$: one view per family, at maximum tier each.

Weighting by tier matters because a claim supported by four views each at
Tier~1 is weaker than one supported by two views each at Tier~4, and a measure
that counts views alone cannot distinguish them.  Harmonic discounting within
families credits redundancy without erasing it: two views from the same family
share a failure mode, so the second contributes less than an independent one
would, and more than nothing.

\subsection{Why Convergence Licenses Realism}\label{sec:why-realism}

Invariance depth can be read at three levels.  The first two are grounded in
established philosophical traditions; the argument is at the third and in the
combination.

\paragraph{Coherentist confirmation.}
Views whose failure modes do not overlap function as conditionally independent
evidence families, and convergence across independent evidence raises posterior
probability \citep{howson1989}.  This is the logic of Wimsatt's robustness
analysis \citep{wimsatt1981} and Stegenga's multimodal evidence
\citep{stegenga2009}.  At this level, high $\delta^{*}$ means high confidence.

\paragraph{Structural realism.}
Structural realism \citep{worrall1989,ladyman1998} holds that science discovers
structures: mathematical relations preserved across theory changes.  In
transformers the relevant structures are those preserved under
computation-preserving weight symmetries, the gauge orbits of
\citet{elhage2021}.  At this level, invariance depth identifies \emph{what} the
views are converging on: the gauge-invariant structure constant across
view-relative descriptions.

\paragraph{The warrant.}
Perspectival realism \citep{massimi2022} holds that scientific knowledge is
perspective-dependent while the states of affairs it tracks are
perspective-independent.  Our extension is that cross-view convergence is
constitutive of the \emph{epistemic warrant} for treating a mechanism as
perspective-independent.  Convergence is constitutive of the warrant, not of
the thing.  The mechanism is real in the structural-realist sense, a
gauge-invariant structure that exists whether or not anyone measures it.
Convergence is the criterion for believing you have found one.

The position is not circular, because ``real'' and ``convergent'' are not
identified.  Convergence is the epistemic entitlement to call something real,
and not the definition of being real.  The mechanism existed, or did not, before
it was measured; high $\delta^{*}$ is the condition under which asserting its
existence is warranted.

The three levels require each other.  Coherentism specifies how much to believe
and supplies no account of what the belief is about.  Structural realism
supplies the ontology and stays silent on how to discover which structures are
there.  The warrant argument fills the gap: convergence across independent
failure modes is the criterion for when evidence licenses treating an invariant
as real.

The restraint this imposes is categorical rather than a matter of degree.  A
Bayesian could model any threshold as a sufficiently high posterior, and under
that reading a claim with $\delta^{*} = 0.25$ is real with low credence.  Under
ours it is categorically a candidate, since it has not met the conditions for
being treated as a mechanism.  The reason is structural: each view has a
characteristic failure mode that no amount of within-view evidence can rule out,
and only evidence from a view with a different failure mode controls for it.

\subsection{Thresholds}\label{sec:thresholds}

Invariance depth is graded, and ontological commitment is licensed at
thresholds.

\begin{center}
\begin{tabular}{cl}
\toprule
$\delta^{*}$ & \textbf{Inference licensed} \\
\midrule
$< 0.25$ & No claim stated \\
$[0.25, 1)$ & Within-view prediction (single view, partial evidence) \\
$[1,2)$, one family & Robustness within a paradigm \\
$[1,2)$, distinct families & Cross-paradigm generalization \\
$\geq 2$, distinct families & Realism inference licensed \\
\bottomrule
\end{tabular}
\end{center}

\subsection{Two Audits}\label{sec:two-audits}

The induction head draws support from four views spanning three families: object
and role from the identity family, structural from the mathematical family, and
process.  Weighting each by family independence and within-view tier gives
$\delta^{*} = 2.875$, which clears the threshold at which the realism inference
is licensed.  High-tier structural evidence, with composition scores visible
directly in weight space, is rewarded; the object and process claims are
discounted to their tier.

A single SAE dictionary element is supported under the object view alone, at low
tier, since a single training run without cross-seed replication is weak evidence
even by that view's own standards.  This yields $\delta^{*} = 0.25$, below the
threshold.  One view, one failure mode, and that failure mode empirically
realized \citep{leask2025}.  The realism inference is not licensed, and the
claim stands as a candidate.

\subsection{View Dynamics}\label{sec:dynamics}

When a view is abandoned for a specific claim, $\mathrm{supp}(C)$ shrinks and
$\delta^{*}$ decreases.  Claims left with an empty support set are retracted;
claims retaining support survive weakened.  View abandonment is relative rather
than global: a view is abandoned for a claim when that claim's discriminating
experiment fails under that view.

The dynamics are asymmetric.  Introducing a new view gives existing claims a
potential boost, since a claim that survives restatement under a newly available
view gains depth it did not have before.  Field progress is therefore measurable
not only by new claims, but by new views that retroactively raise the depth of
existing ones.


\section{Researcher Degrees of Freedom Across Views}\label{sec:forking}

Invariance depth is only meaningful if the view-support set was specified before
the views were run.  Otherwise it records how many views could be found that
agreed, which is a different quantity.

% SKELETON. The framework is vulnerable to the failure mode it diagnoses: running
% nine views and reporting the ones that agreed is a garden-of-forking-paths
% statistic, and an instance of the "flexible scope" and "circuit redefinition"
% failure modes catalogued elsewhere in this literature.
% The fix has four parts:
%   1. A PREREGISTERED view-support set is severe; a DISCOVERED one is not. This is
%      the confirmation/corroboration distinction applied to views rather than to
%      criteria, and it is the same severity rule used in the validity framework.
%   2. Report the specification curve across views - every view's result for a claim -
%      rather than max delta or the supporting subset. Import from multiverse /
%      specification-curve analysis; cf. its use in ML to prevent fairness hacking
%      ("One Model Many Scores", FAccT 2024).
%   3. Null baselines: support rates for random directions and scrambled claims, so
%      "how often does this fire by chance across N views" is measured, not assumed.
%   4. Benjamini-Hochberg control across claims within each view.

\section{Empirical Test}\label{sec:experiment}

% PLACEHOLDER - blocked on the experiment being run.
% Preregistration: PREREG_cross_view_predictive_validity.md (freeze SHA pending).
% Central claim: cross-view support predicts mechanistic correctness beyond the best
% single-view result, the number of methods used, and ordinary faithfulness.
% Testbed: Tracr programs (ground truth by construction) plus learned modular-arithmetic
% models for the process view. IOI is excluded - no undisputed mechanism-level ground truth.
%
% PROMOTION TARGET (confirmed by literature search): SAE features.
% They are genuinely single-view (object), no weight-space composition analysis of
% decoder directions exists, and there is now a PUBLISHED RESCUE HYPOTHESIS to test:
%   "Unstable Features, Reproducible Subspaces" (arXiv 2606.12138) - individual SAE
%     features are not reproducible across seeds, but they cluster into reproducible
%     low-rank SUBSPACES. This is precisely "the object view fails, the subspace view
%     may rescue it," and makes the promotion experiment concrete: does promotion to
%     the subspace view survive where the object view does not?
%   "Ablating Archetypes" (arXiv 2606.02061) - claimed stability gains were an artifact
%     of shared initialisation, not genuine stabilisation.
%   "Tracking the Feature Dynamics in LLM Training" (arXiv 2412.17626) - checkpoint-level
%     feature formation, the process-view arm.
% J-space was considered and rejected as the calibration target: code is public
% (github.com/anthropics/jacobian-lens) and Nanda replicated it on Qwen 3.6 27B with
% ~25 prompts, but it is weeks old, has no ground truth, and no stabilised baseline
% claim - the first structural analysis of it would have no comparison point.
...
\newblock In M.~B. Brewer and B.~E. Collins, editors, \emph{Scientific Inquiry
  and the Social Sciences}, pages 124--163. Jossey-Bass, 1981.

\bibitem[Stegenga(2009)]{stegenga2009}
J.~Stegenga.
\newblock Robustness, discordance, and relevance.
\newblock \emph{Philosophy of Science}, 76(5):650--661, 2009.
...
\bibitem[Ladyman(1998)]{ladyman1998}
J.~Ladyman.
\newblock What is structural realism?
\newblock \emph{Studies in History and Philosophy of Science Part A},
  29(3):409--424, 1998.

\bibitem[Levi(1980)]{levi1980}
I.~Levi.
\newblock \emph{The Enterprise of Knowledge}.
\newblock MIT Press, 1980.

\bibitem[G\"ardenfors(1988)]{gardenfors1988}
P.~G\"ardenfors.
\newblock \emph{Knowledge in Flux: Modeling the Dynamics of Epistemic States}.
\newblock MIT Press, 1988.

\bibitem[Lawlor et~al.(2016)]{lawlor2016}
D.~A. Lawlor, K.~Tilling, and G.~Davey~Smith.
\newblock Triangulation in aetiological epidemiology.
\newblock \emph{International Journal of Epidemiology}, 45(6):1866--1886, 2016.
...
Elliot Tower.
\newblock Mechanistic Validity: Evidence Standards for Mechanism Claims.
...
%% ---------------------------------------------------------------
\section{The Nine Views in Detail}\label{app:viewdetail}
%% ---------------------------------------------------------------
