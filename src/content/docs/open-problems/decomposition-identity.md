...
This is the decomposition identity problem, and it is arguably the most consequential open question in mechanistic interpretability. Anthropic's scaling monosemanticity program (Templeton et al. 2024) trained SAEs at massive scale on Claude 3 Sonnet and found millions of interpretable features. But the features you find depend on the dictionary width, the sparsity penalty, and the training procedure. Change these hyperparameters and you get a different decomposition of the same model. The features are not unique.
