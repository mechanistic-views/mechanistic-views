...
You have run activation patching on GPT-2 Small's IOI circuit and found that head 9.9 is necessary for name-moving. Clean result, large effect size. Can you publish the claim "head 9.9 implements name-moving"?

With activation patching alone, several alternative explanations remain live. Head 9.9 might be necessary because it is part of a backup circuit that engages when the primary mechanism is disrupted. Its necessity might be an artifact of the specific prompt distribution you tested on. And you have no evidence about *how* the head implements name-moving -- only that removing it breaks things. The claim is supported but underdetermined: multiple distinct mechanisms are consistent with the same activation patching results.
...
**What it commits you to.** Stating explicitly which failure modes would overturn the claim and what cross-domain tests would confirm it. A single-method claim should be framed as preliminary: "activation patching shows head 9.9 is necessary for IOI under this prompt distribution."
...
The partition that matters is by *failure mode*, not by data type. Two views belong to the same family when one methodological error corrupts both. The five families are: **identity** (object, role — a role asserted rather than operationalized), **mathematical** (subspace, structural — vacuous alignment), **process** (process — training overfitting), **pragmatic** (instrumental — safety-relevant overreach), and **analyst-choice** (contrastive, perspectival, stratified — a parameter the analyst sets, mistaken for the mechanism). Grouping by subject matter instead would let two views with a common blind spot appear to corroborate each other. The evidence domains below cut across these families and are useful for planning experiments, not for scoring a claim:
...
**One family**: the claim is a **candidate**. A structure supported inside a single family may well be real; the criterion records that the evidence has not yet excluded that family's characteristic artifact, and names which measurement would.

**Two families**: the claim is **view-invariant**, provided no applied view yields an undefeated incompatible result. Two is the minimum the argument requires rather than a tuned threshold — within one family a single characteristic artifact could produce the whole agreement.

**Incompatible results across applied views** that no difference in grain or identity criterion reconciles: the claim is **contested**.

Counting agreeing methods is not a substitute for crossing a family boundary. Ontically independent lines of evidence can jointly confirm a hypothesis less than each does alone (Stegenga, 2017), so a tally measures the wrong quantity. What a claim should carry is its view-support set: which views were tried, which agreed, and which families they fall in.
