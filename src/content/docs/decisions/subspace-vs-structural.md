...
Two mechanisms are the same when their subspaces are close on $\mathrm{Gr}(k,d)$.
...
Two mechanisms are the same when they lie in the same gauge orbit.
...
Subspace view for single-model claims and preliminary cross-model comparisons, with gauge normalization applied. Structural view (with holonomy and generic-position check) for strong cross-model identity claims where computational cost is acceptable.
