...
The role graph and the [directed graph](/mechanistic-views/formalism/directed-graph/) are structurally identical (both are DAGs). The difference is semantic: directed graph nodes are component identities (head 9.9), role graph nodes are functional labels (name-mover). The [causal graph](/mechanistic-views/formalism/causal-graph/) is more abstract still — its nodes are high-level causal variables (subject gender), not model components or functional roles. A concrete example: the IOI circuit can be described at all three levels — the same computation appears as a directed graph of specific heads, a role graph of functional labels (name-mover, S-inhibition), or a causal graph of high-level variables (subject, indirect object).
