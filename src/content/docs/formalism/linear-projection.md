...
The limitation: observational only. A [logit lens or tuned lens](/mechanistic-views/glossary/#logit-lens) reading shows what information is linearly accessible at each layer, not what the model actually uses. Two layers can show the same logit lens output while playing different causal roles in the computation.

## Relationship to other formalisms

The unembedding projection $W_U$ defines a preferred frame for reading off vocabulary-level predictions. The [Grassmannian](/mechanistic-views/formalism/grassmannian/) formalism generalizes this: instead of projecting onto the unembedding basis, it considers arbitrary subspaces of the residual stream. The [linear classifier](/mechanistic-views/formalism/linear-classifier/) is another observational formalism, but tests for binary concept presence rather than vocabulary-level readout. See the [methods page](/mechanistic-views/methods/) for how [logit/tuned lens](/mechanistic-views/glossary/#logit-lens) fits into the evidence landscape.
