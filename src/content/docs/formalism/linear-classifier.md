...
The limitation: linear accessibility is observational, not causal. A probe can achieve high accuracy on information the model does not actually use in its computation. Probing establishes that information is *present*, not that it is *causally active*. Causal methods like [DAS](/mechanistic-views/glossary/#causal-abstraction) or [activation patching](/mechanistic-views/glossary/#activation-patching) are needed to establish the latter.

## Relationship to other formalisms

A linear probe recovers a hyperplane normal — a point in $\mathrm{Gr}(1, d)$ — which can be compared to [SAE features](/mechanistic-views/formalism/dictionary/) or [DAS subspaces](/mechanistic-views/formalism/grassmannian/) in the same space. The key difference is in the evidence type: probing is observational (what is present), while DAS/IIA is interventional (what is causally active). See the [methods page](/mechanistic-views/methods/) for where [linear probing](/mechanistic-views/glossary/#linear-probing) sits in the evidence landscape.
