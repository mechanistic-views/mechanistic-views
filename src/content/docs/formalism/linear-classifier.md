...
A linear probe recovers a hyperplane normal — a point in $\mathrm{Gr}(1, d)$ — which can be compared to [SAE features](/mechanistic-views/formalism/dictionary/) or [DAS subspaces](/mechanistic-views/formalism/grassmannian/) in the same space. The key difference is in the evidence type: probing is observational (what is present), while DAS/IIA is interventional (what is causally active). See the [methods page](/mechanistic-views/methods/) for where [linear probing](https://learnmechinterp.com/topics/probing-classifiers/) sits in the evidence landscape.
