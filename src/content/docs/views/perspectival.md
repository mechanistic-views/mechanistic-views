...
The philosophical lineage runs through Wimsatt (1981): robustness analysis keeps what is invariant across several independent detection processes. Multiple detection is what carries the ontology — a finding that survives structurally different measurements is more likely to track something real than one that depends on a single method. Massimi's *perspectival realism* (Oxford, 2022) is the nearest position in contemporary philosophy of science, and it takes the opposite ontological route: it is realist about modally robust phenomena and explicitly not about the causal properties beneath them (pp. 15, 189), where a mechanism claim is a claim about exactly those. The perspectival view differs from relativism: it does not say that any description is as good as any other. Descriptions are evaluated within their measurement context, and convergence across contexts is stronger evidence than any single-context result.

A critical distinction: **perspectival evidence** (measurements are always from a vantage point) versus **perspectival truth** (what is true depends on who measures). The perspectival view commits to the first and is agnostic about the second. The primary failure mode is conflating the method-relativity of evidence with the claim that mechanisms are themselves method-relative objects. That evidence is method-relative is uncontroversial; that the things evidence is about are method-relative is a much stronger claim, and one the perspectival view need not make.
...
- Wimsatt, "Robustness, Reliability, and Overdetermination" (1981) — the view's identity criterion: keep what is invariant across several independent detection processes, and explain the failures of invariance rather than discarding them
- Massimi, *Perspectival Realism* (Oxford, 2022) — the nearest contemporary position, and one that takes the opposite ontological route
- For comparison: the [instrumental view](/mechanistic-views/views/instrumental/) is anti-realist about mechanisms entirely, while the perspectival view holds that the evidence is method-relative and stays agnostic about whether mechanisms themselves are. The [subspace](/mechanistic-views/views/subspace/) and [structural](/mechanistic-views/views/structural/) views are realist — they commit to specific objects that exist independently of any measurement procedure
