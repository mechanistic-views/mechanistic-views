...
Most published interpretability work operates at the Object or Role level without stating this explicitly. This creates confusion when papers using different views disagree — the IOI circuit replication debates, the probing wars, and "how big is a circuit?" arguments are all partly view-level disagreements masquerading as empirical ones. Making the view explicit takes one sentence and prevents a class of errors where evidence collected at one level is used to draw conclusions at another. See [Open Problems](/mechanistic-views/open-problems/) for a systematic analysis of how current open problems in MI map onto view confusions.

The views apply to both **mechanisms** (circuits, computations) and **features** (the representations those circuits operate over). A feature is a mechanism at a different grain: the Object view says a feature is a specific direction or neuron, the Role view says it's whatever plays a given functional role, the Subspace view says it's a causal subspace, and so on. Most of the field's feature-level debates — [do SAEs find "true" features?](/mechanistic-views/open-problems/sae-true-features/), [do probes find features the model uses?](/mechanistic-views/open-problems/probe-features/), [is superposition a problem or a property?](/mechanistic-views/open-problems/superposition/) — are view confusions at the feature level.
...
The pattern: the object view gives you necessity and sufficiency but cannot establish cross-architecture identity, measurement invariance, or convergent validity — these are structurally impossible, not just untested. The subspace view resolves the coordinate-dependence problems (invariance, convergence) and has zero impossible criteria. The structural and stratified views have the broadest coverage but are computationally expensive.
