...
The triangulation requirement in this site — requiring convergent evidence across weight-space, activation-space, and dynamics-space domains — is a response to underdetermination. The single-domain evidence produces an underdetermined verdict; multi-domain convergence narrows the hypothesis space. This is a pragmatic application of the principle that *independent* evidence is stronger than repeated evidence from the same source. See [Single-method vs triangulated](/mechanistic-views/decisions/single-vs-triangulated/).
